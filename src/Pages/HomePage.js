...
export default function HomePage() {
  ...
  //   const interval = setInterval(clear, 100);
    ...
    <div>
      ...
      {isLoggedIn ? (
        <>
          ...
          <p>
            Want to jump straight to <Link to="contacts">Phonebook</Link>?
          </p>
        </>
      ) : (
        <p>
          In order to use personal contacts app you need to{" "}
          <Link to="login">Log In</Link>. Or if you are a newcomer please fell
          free to <Link to="signup">Sign Up</Link>
        </p>
      )}
      <p>Or take some time to play with our magic background :)</p>
      <canvas id="c"></canvas>
    </div>
  );
}
